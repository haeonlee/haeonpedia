...
int main(void)
{
    ...
    while (1)
    {
        if ( fgets(new_word, sizeof(new_word), bfp) == NULL ) break;    // 입력 단어 파일 다 훑으면 끝
        // 여기서 fgets 할 버퍼 하나가 소비돼서 bfp 파일에서 제대로 get이 안 됐어용
        fgets(new_word, sizeof(new_word), bfp);

        for (i=0; i<10; i++)
        {
            if ( re_word[i] == new_word ) break;
           
            else { fputs(new_word, cfp); }
        }
