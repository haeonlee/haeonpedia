...
matrixPtr madd(matrixPtr A, matrixPtr B) {
    ...
    // 1. 필요한 변수 및 함수를 선언합니다.
    // 이때, COMPARE 함수는 후에 A+B 연산을 할 때 사용됩니다.
    ...
    // 2. matrix A와 B의 데이터를 row 단위로 받아읽으며, matrix C를 생성합니다.

    nRows = A->u.entry.row;
    nCols = A->u.entry.col;

    // (1) matrix C, 다시 말해 A+B의 데이터를 저장할 main head node를 생성합니다.
    // 이때, val 값(C의 terms 개수)는 A+B 연산을 하며 정확하게 수정할 예정입니다.
    ...
    node->u.entry.val = A->u.entry.val + B->u.entry.val;
    ...
    // (4) row 기준으로 matrix A, B의 데이터를 읽고, matrix C를 만듭니다.
    // - matrix C의 entry node들을 동적 할당합니다.
    // - node들을 row/col 기준으로 각각 연결합니다.
    ...
    for (int r = 0; r < (A->u.entry.row); r++)  // row 기준으로 데이터 입력
    {
        ...
        while ((Atmp != Ahead) && (Btmp != Bhead))
        {
            ...
            case -1:    // Atmp의 col 값 < Btmp의 col 값
                tmp = newNode()
                tmp->tag = entry;
                tmp->u.entry.row = Atmp->u.entry.row;
                tmp->u.entry.col = Atmp->u.entry.col;
                tmp->u.entry.val = Atmp->u.entry.val;
                Atmp = Atmp->right;     // 계산을 마쳤다면, Atmp는 다음 노드로 이동
                break;

            case 0:     // Atmp의 col 값 = Btmp의 col 값
                // Atmp의 val 값 + Btmp의 val 값이 0이 아닐 때만 matrix C에 저장
                if ((Atmp->u.entry.val) + (Btmp->u.entry.val))
                {
                    ...
                    tmp->u.entry.row = Atmp->u.entry.row;
                    tmp->u.entry.col = Atmp->u.entry.col;
                    tmp->u.entry.val = Atmp->u.entry.val + Btmp->u.entry.val;
                }
                Atmp = Atmp->right;         // 계산을 마쳤다면, Atmp는 다음 노드로 이동
                Btmp = Btmp->right;         // 계산을 마쳤다면, Btmp는 다음 노드로 이동
                node->u.entry.val -= 1;     // 중복 계산된 C의 terms 개수를 감소합니다.
                break;

            case 1:     // Atmp의 col 값 > Btmp의 col 값
                tmp = newNode()
                tmp->tag = entry;
                tmp->u.entry.row = Btmp->u.entry.row;
                tmp->u.entry.col = Btmp->u.entry.col;
                tmp->u.entry.val = Btmp->u.entry.val;
                Btmp = Btmp->right;     // 계산을 마쳤다면, Btmp는 다음 노드로 이동
                break;
            }
            ...
            // (2) col 기준으로 연결 1
            hdnode[tmp->u.entry.col]->u.next->down = tmp;    // col 기준으로 연결
            hdnode[tmp->u.entry.col]->u.next = tmp;          // 이때, hdnode[tmp->u.entry.col]->u.next는 tmp,
                                                             // 즉, 해당 행에서 마지막으로 입력된 node를 가리킵니다.
                                                             // 이는 임시적(temporal)인 setting입니다.
        }

        // A, B에서 해당 matrix에 아직 노드가 남아 있을 수 있기 때문에, while문을 사용해 남은 노드를 모두 소진
        ...
        // matrix A, B
        // 해당 row에서 모든 연산이 끝났으므로, 다음 row로 이동
        Ahead = Ahead->u.next;
        Bhead = Bhead->u.next;

        // matrix C
        // 해당 row에서 모든 연산이 끝났으므로, 다음 row로 이동
        last->right = hdnode[r];        // circular하게 연결
        last = hdnode[r++];             // 다음 노드로 이동
    }

    last->right = hdnode[nRows-1];
    ...
    /*
    printf("MADD");
    testSMatrix_mid(node, hdnode);
    

    printf("madd에서의 mwrite\n");
    mwrite(node);
    */
...
void mwriteFull(matrixPtr node) {
    ...
    oldRow = 0, oldCol = -1;       // 초기 입력 노드를 의미하는 oldRow와 oldCol은 (0, -1)로 선언합니다.
    ...
    for (int i = 0; i < node->u.entry.row; i++ )
    {
        for ( tmp = head->right; tmp != head; tmp = tmp->right )
        {
            ...
            {
                pDataNum += 1;
                // printf("col: %d, pNum: %d\n", node->u.entry.col, pDataNum);
                if ((pDataNum % (node->u.entry.col)) != 0)
                {
                    printf("%5d ", 0);
                }
                else
    ...
    // (3) 문제는 위 과정을 거치면, last entry node가 해당 matrix를 끝까지 채우지 않은 경우,
    // (예를 들어, [2X2] matrix라면, [row: 2, col: 2]에 데이터가 없을 경우)
    // matrix가 끝까지 출력되지 않는 문제가 발생합니다.
    // 이를 해결하기 위해 지금까지 출력된 data의 개수와 실제 terms 개수를 비교하고,
    // 미처 출력되지 않은 data들을 마저 출력하는 작업을 하겠습니다.
    // 물론, 이때 출력되는 data들은 모두 0일 것입니다.

    if ( pDataNum != node->u.entry.col )
    {
        for ( ; pDataNum <= node->u.entry.col; pDataNum++ )
        {
            pDataNum += 1;
            if ((pDataNum % (node->u.entry.col)) != 0)
            {
