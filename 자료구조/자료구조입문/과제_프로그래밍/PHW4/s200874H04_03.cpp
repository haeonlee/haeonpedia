...
matrixPtr madd(matrixPtr A, matrixPtr B) {
    ...
    // 이때, A+B 연산 값은 동적 할당한 구조체 배열(CArr)에 저장한 뒤, 추후에 사용할 것입니다.

    // (1) 구조체 배열을 동적 할당(Carr)한 뒤,
    //     배열 전체를 (row, col, val)=(-1, -1, -1)로 초기화

    int CArrSize = (A->u.entry.val) + (B->u.entry.val);
    entryNode* CArr = (entryNode*)malloc(sizeof(entryNode) * CArrSize);     // A의 terms 개수 + B의 terms 개수만큼 크기를 동적 할당
                                                                            // CArr에 남는 공간이 생길 순 있지만, 프로그램상 편의를 위해 이렇게 배정
    allocSize += sizeof(entryNode) * CArrSize;
    ...
    i = 0;              // CArr에 데이터를 저장할 때 사용할 index

    int switchNum = COMPARE((Atmp->u.entry.col), (Btmp->u.entry.col));
            ...
            switch(switchNum)
            {
                ...
                case 0:     // Atmp의 col 값 = Btmp의 col 값
                    ...
                    if ((Atmp->u.entry.val)+(Btmp->u.entry.val))
                    {
                        CArr[i].row = Atmp->u.entry.row;
                        CArr[i].col = Atmp->u.entry.col;
                        CArr[i].val = (Atmp->u.entry.val) + (Btmp->u.entry.val);
                        i++;
                    }
                    Atmp = Atmp->right;     // 계산을 마쳤다면, Atmp는 다음 노드로 이동
...
void mwriteFull(matrixPtr node) {
    ...
    if ( pDataNum != node->u.entry.val )
    {
        for ( ; pDataNum <= node->u.entry.val; pDataNum++ )
        {
            pDataNum += 1;
            if ((pDataNum % (node->u.entry.col)) != 0)
            {
